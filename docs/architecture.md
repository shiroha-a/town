...
│   ├── components/    # 画面と部品(57ファイル。うち13がカジノ)
...
| `stock` | 株取引場(5銘柄・共有価格) |
| `keiba` | 競馬 |
| `casino` | 単発のカジノゲーム8種(サイコロ・スロット・くじ等。純粋関数として実装)とカード引きの卓の規則 |
...
- `components/` — 施設ごとの画面(BankView, DepartView, …)と共通部品
  ...
  - `casino/` — カジノの各ゲーム(単発8種 + スクラッチ・ブラックジャック・ポーカー・ロト6・カード引き)
...
| どこ | 何を持つか | 変更方法 |
| --- | --- | --- |
| `backend/default.yml` | インフラのみ(DB/Redis/待ち受けポート/公開アドレス/worker周期) | `.example` をコピーして編集。**任意**で、環境変数だけでも起動する |
...
設定ファイルとcomposeのoverrideはどちらもリポジトリに `.example` だけを置き、
実ファイルは環境ごとに作ります(公開アドレスや暗号鍵が入るため)。コンテナは
設定ファイルを同梱せず、環境変数だけで起動します。
